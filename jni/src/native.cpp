...
static void engine_draw_frame(Engine* engine, const cv::Mat& frameIN)
{
	cv::Mat frame;
	if(frameIN.channels() == 1)
		cv::cvtColor(frameIN,frame,CV_GRAY2RGBA);
	else
		frame = frameIN;
    ...
    switch (cmd)
    {
        case APP_CMD_INIT_WINDOW:
            if (app->window != NULL)
            {
                ...
                float scale = std::min((float)view_width/(camera_resolution.width + 150),
...
void android_main(android_app* app)
{
    ...
    cv::Mat* UI_frame;
    unsigned int grabbedFramesCount = 0;
    PREVIeW PREVIeW_engine;
	UI_frame = PREVIeW_engine.createUI();
	engine_draw_frame(&engine, *UI_frame);
	if(!PREVIeW_engine.readMetadata()) return;
    ...
    while (1)
    {
        ...
        if (!engine.capture.empty())
        {
             ...
             if(grabbedFramesCount>5)
             {
            	 UI_frame = PREVIeW_engine.addImage(drawing_frame);

	             char buffer[256];
	             sprintf(buffer, "Display performance: %dx%d @ %.3f", UI_frame->cols, UI_frame->rows, fps);
	             cv::putText(*UI_frame, std::string(buffer), cv::Point(8,64),
